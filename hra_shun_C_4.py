...
def score(index,sim,loop=100):
    ...
    if loop > 0:
        print total_reward,
    print "epailon:%2.2e" % agent.get_epailon(),
    ...
    fvs.write("\n");
    fvs.close()
    out="%d,%2.2e,%2.2e,%2.2e\n" % (index,total_reward,agent.get_epailon(),agent.outloss)
    fw.write(out)
    fw.flush()
...
class DQNAgent():
    def __init__(self, args, epailon=1.00):
        self.model = Q()
        ...
        self.epailon = epailon # ランダムアクションを選ぶ確率
    ...
    def reduce_epailon(self):
        pass
        #self.epailon-=1.0/100000
        #if self.epailon<0.05:
        #    self.epailon = 0.05

    def get_epailon(self):
        return self.epailon
    ...
    def get_action(self,seq,train):
        ...
        '''
        action=0
        if train==True and np.random.random()<self.epailon:
            # random
            dummyactions = []
...
class pendulumEnvironment():
    ...
    def get_shuntu_reward(self,first_pai):
        ...
        return (self.shuntu[first_pai] - oldshuntu)*10
...
class simulator:
    ...
    def run(self, train=True, movie=False, enableLog=False):
        ...
        for i in range(27):
            ...
            #for p in PAI_NUM2ACT:
            #    reward.append(self.env.get_koutu(p))
