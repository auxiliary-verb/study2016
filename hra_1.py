...
class Q(Chain):
    def __init__(self,state_num=INPUT_NODE):
        super(Q,self).__init__(
             l1=L.Linear(state_num, 8),  # stateがインプット
...
class pendulumEnvironment():
    ...
    def get_shuntu(self):
        if self.tehai[1] > 0 and self.tehai[2] > 0 and self.tehai[3] > 0:
            return 1
        return 0
...
class simulator:
    ...
    def run(self, train=True, movie=False, enableLog=False):
        ...
        for i in range(27):
            ...
            if movie:
                self.agent.get_action_value(old_seq,True)
            action = 0
